  ...
  return (
    <>
      ...
      <div className="detail-grid">
        ...
        <section className="card"><h2>Linked Documents</h2>{documents.map((doc) => <div className="list-row" key={doc.id}><div><strong>{doc.fileName}</strong><span>{doc.size} · {doc.uploadedBy}</span></div><Paperclip size={18}/></div>)}<button>Attach Document</button></section>
      </div>
